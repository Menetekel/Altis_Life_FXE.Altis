...
switch (life_veh_shop) do
{
	case "civ_car_1":
	{
		if(playerSide != civilian) exitWith {_kill = "Du bist kein Zivilist!"};
	...
	case "cop_car_1":
	{
		if(playerSide != west) exitWith {_kill =  "Du bist kein Polizist!"};
	...
	{
		if(playerSide != west) exitWith {_kill =  "Du bist kein Polizist!"};
		if(__GETC__(life_adminlevel) < 1) exitWith {_kill = "Du bist nicht vom SEK!"};
	...
	case "donator_heli":
	{
		if(__GETC__(life_donator) < 1) exitWith {_kill = "Du hast nicht gespendet!"};
