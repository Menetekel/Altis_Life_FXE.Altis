...
_pgText ctrlSetText format["Helikopter-Wartung  (1%1)...","%"];
...
while {true} do
{
	...
	_pgText ctrlSetText format["Helikopter-Wartung (%1%2)...",round(_cP * 100),"%"];
	if(_cP >= 1) exitWith {};
};

if(!alive (_search select 0) || (_search select 0) distance air_sp > 10) exitWith {life_action_inUse = false; hint "Es steht kein Helikopter auf dem Helipad."};
...
5 cutText ["","PLAIN"];
titleText ["Dein Helikopter ist nun repariert und aufgetankt.","PLAIN"];
