...
_spikeStrip = "Tapesign_f" createVehicle [0,0,0];
_spikeStrip attachTo[player,[0,5.5,0]];
_spikeStrip setDir 90;
_spikeStrip setVariable["item","spikeDeployed",true];

life_action_spikeStripDeploy = player addAction["Plaziere Flatterband",{if(!isNull life_spikestrip) then {detach life_spikeStrip; life_spikeStrip = ObjNull;}; player removeAction life_action_spikeStripDeploy; life_action_spikeStripDeploy = nil;},"",999,false,false,"",'!isNull life_spikestrip'];
...
life_action_spikeStripPickup = player addAction["Baue Flatterband ab",life_fnc_packuptapesign,"",0,false,false,"",
' _spikes = nearestObjects[getPos player,["Tapesign_f"],8] select 0; !isNil "_spikes" && !isNil {(_spikes getVariable "item")}'];
